 ...
 *      判据在 scripts/brand-eol-pin-lib.js（2026-09-21 的真实缺陷：漏钉 ⇒ 主仓库绿、干净 clone 红）。
 *
...
const eolPin = require('./brand-eol-pin-lib.js');
...
// 判据（属性解析 / 匹配语义 / 判定 / 路径集合推导 / 输出）在 scripts/brand-eol-pin-lib.js：
// 那个模块零依赖、数据由调用方注入，所以测试能把它逐分支钉住（含与 `git check-attr` 的差分）。
// 本脚本是整仓装配型（要一整棵真实资产树才能跑）：把判据拆出去，测试就不必加载整份 CLI，
// 也就不必为了"把分母里的未执行行凑掉"而写假覆盖。这里只做注入 + 把失败接到 fail()（⇒ 退出码 1）。
function checkEolPinning() {
  return eolPin.checkEolPinning({
    root: ROOT,
    mapping: brand.MAPPING,
    exportDir: brand.V2_EXPORT_DIR,
    io: { pass, fail, log: (line) => console.log(line) },
  });
}
