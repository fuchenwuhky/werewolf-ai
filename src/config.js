...
const DEFAULT_CONFIG = {
  ...
  // 单局 AI 调用次数硬上限（P1 真实对局测试实测后加的护栏）。
  // 依据：真实对局实测平均 55.4 次/局、最大 161 次/局（23 局样本）；
  // 而注入"僵持"故障时，秒回的模型 25 秒就把单局刷到近 6000 次 —— 时间预算（180s/决策）拦不住按次计的成本。
  // 400 ≈ 实测最大值的 2.5 倍，正常对局永远碰不到；碰到即暂停（可恢复、不判负）并留下排查现场。
  maxCallsPerGame: 400,
  cacheControl: false,
