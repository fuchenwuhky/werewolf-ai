...
/**
 * 回归测试（FIX-01b）：`web/sw.js` 的 install 把预缓存写进 `SHELL_CACHE`，而静态资源的离线兜底
 * 原先只查 `ASSET_CACHE` —— 于是「装完 SW、还没联网加载过带版本查询串的子资源就断网」时，页面外壳
 * 能打开（导航查的正是 SHELL_CACHE），它引用的 `app.js?v=<哈希>` 却查不到，注释里宣称修好的那条
 * P1 在这条路径上并不成立。
 *
 * 历史：该缺口由 FIX-21 补测时发现（当时 sw.js 定稿不久、本文件无权修改），先以 todo 形式登记成
 * 「看得见的缺口」；随后在 FIX-01b 修掉 —— 兜底改为在 ASSET_CACHE 与 SHELL_CACHE 两个仓里依次
 * 精确匹配、再忽略查询串匹配。本条因此转为正式回归断言，不再带 todo 标记。
 */
test('装完 SW 就断网：子资源应当能从预缓存（SHELL 仓）命中', async () => {
  const site = makeSite();
  const { sw } = await bootedSw(site);
