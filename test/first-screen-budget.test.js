 ...
 *      FIX-13 之后 `depsUnchanged()` 先只比对 stat（size + mtimeMs），**stat 未变就一次盘都不读**；
 *      只有 stat 变了才重读重哈希（内容变必然伴随 stat 变，所以这个短路是安全的）。
 *
 ...
 *   · 暖路径单次 HTML 请求（阈值 **min < 50 ms、p50 < 200 ms**，一直没动）：
 *     — 改造前（FIX-21 记录）：min 2.2~2.6 ms、p50 3.3 ms；争用态 min 3.4 ms、p50 5.7 ms；
 *     — **FIX-13 之后实测：min 0.7 ms、p50 1.0 ms**（`/index.html` 与 `/m/index.html` 同值），
 *       约 3 倍改善；争用态数值仍是改造前所测 —— 保留它作为阈值余量的**保守**依据（改造后余量只会更大）。
 ...
 *
 * ## 拦不住什么（如实标注边界）
 *   · **10 倍以内的退化基本拦不住**：本机基线只有亚毫秒级（暖路径，FIX-13 后实测 min 0.7 ms）
 *     与 0.14 s（冷启动），为保证慢机器与并行争用下绝不假红，阈值留到了几十倍；因此 2~5 倍、
 *     乃至 10 倍的性能回退都在阈值内，**不会红**；
 *   · 反过来说：**FIX-13 把暖路径基线拉低之后，这条护栏能抓到的"相对退化倍数"反而更小了** ——
 *     它的阈值锚的是绝对毫秒数（min 50 ms / p50 200 ms），基线越低，"落在阈值内的倍数"就越大。
 *     真要抓小倍数回退，得靠 `test/static-cache-invalidation.test.js` 那类**行为断言**（直接数
 *     读盘次数、不依赖墙钟时间），而不是靠这条时间护栏 —— 这是分工，不是缺陷；
