| FIN-13 | 已完成 | artifacts/acceptance/1.5.2-cf024ab/ | packages.md 三制品 sha256；app:verify EXIT=0 | 后续任何运行时/资产变更需重建重验 || FIN-12 | 部分完成 | artifacts/acceptance/1.5.2-cf024ab/ | acceptance-results.md C/R/V 逐条；ui:check --strict exit 0；MuMu 安装/升级/启动全过 | Android 物理真机与 V11/V12 专项保持「待验证」，不签整体 || FIN-10 | 已完成（D+M） | 本批评见 git log | 座位增量 patch/直播骨架化/滚动锚点（D 报告修复 smooth-scroll 脱钩） | 500/2000/6000 事件压测未执行 || FIN-09 | 已完成（R） | 本批评见 git log | app:verify EXE 逐帧+RT_VERSION；包内二进制 0 个只查存在 | 资源管理器/任务栏视觉属真机项 || FIN-08 | 已完成 | 本批评见 git log | brand:check 62 项（wolf-emblem svg-master+img 引用核对）；双端页面引用截图 | — || FIN-07 | 已完成（Q） | 17dcea3 系列 | fin07-contracts.test.js +11；十二行核验结论见下表 | 草稿断网/偏好应用/旧 SSE 释放的浏览器侧已抽样 || FIN-06 | 已完成（M） | 本批评见 git log | ui:check 手机段全绿；三页签/返回栈/安全区/键盘补偿；截图 V08 | 实体键盘/安全区真机项待验 || FIN-05 | 已完成（D） | 本批评见 git log | 截图 V07；1100px 抽屉实测；圆桌/列表切换 | 圆桌 240px 栏内目标区合规由列表视图承担（注释说明） || FIN-04 | 已完成（D+M） | 本批评见 git log | 截图 V01/V08：双端首页新信息架构；结算分层；设置四分组 | 字号/布局偏好应用未接（无假开关） || FIN-03 | 已完成（D+M） | 本批评见 git log | ui:check --strict 全绿（exit 0）；双端触区 token/composing/防双击/模态焦点 | 物理真机触区留待真机 |# 最终交付施工台账（FIN-00）
